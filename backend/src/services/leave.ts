...
function validateInput(input: LeaveInput, allowPast: boolean) {
  ...
  if (input.duration === 'full_day' && !['sick', 'personal'].includes(input.leaveType ?? ''))
    throw badRequest('กรุณาเลือกประเภทลาป่วยหรือลากิจ')
  if (input.duration !== 'full_day' && input.leaveType !== null) throw badRequest('การลาครึ่งวันไม่ต้องเลือกประเภทลา')
